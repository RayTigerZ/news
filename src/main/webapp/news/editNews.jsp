...
<html>
	...
	<script type="text/javascript">	
		...
		function cancel(){//取消修改 
			ids1=document.getElementById("ids"); 
		  	ids1.value="";
		  	//提交
		  	document.getElementById('myform').action="/news/servlet/NewsServlet?type=manageNews";
  ...
  <body>
   <div class="div-out"> 
	<form action="/news/servlet/NewsServlet?type=edit&newsId=${requestScope.news.newsId}" 
				id="myform" method="post" onsubmit="return submit1()">
		<table width="800" align="center" border="1">
			<tbody>
				<tr><td>标题：</td><td>
					<input type="text" size="100" value="${requestScope.news.caption}"  name="caption" id="caption"></td>
				</tr>
				<tr><td>类型：</td><td>
						<select name="newsType" id="newsType">
						<c:forEach items="${applicationScope.newsTypes}"  var="newsType">
							<c:choose>
							    <c:when test="${newsType.newsTypeName == requestScope.news.newsType}">
							       	<option value="${newsType.newsTypeName}" selected>${newsType.newsTypeName}</option>
							    </c:when>
							    <c:otherwise>
